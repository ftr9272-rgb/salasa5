...
const SupplierLayout = ({ children, activeTab, setActiveTab, onLogout, onGoToMarketplace }) => {
  ...
  const navItems = [
    ...
    // Partners is a parent item that contains two subitems: merchants and shipping companies
    { id: 'partners', name: 'الشركاء', icon: Users, children: [
      { id: 'partners-merchants', name: 'التجار', icon: Users },
      { id: 'partners-shipping', name: 'شركات الشحن', icon: Factory }
    ]},
  ...
  <div className="flex flex-col h-full overflow-hidden">
          ...
          <nav className="flex-1 p-4 space-y-2 overflow-auto">
            {navItems.map((item) => {
              ...
              // If this nav item has children, render a parent with expandable children
              if (item.children && item.children.length > 0) {
                const isParentActive = item.children.some((c) => activeTab === c.id);
                return (
                  <div key={item.id} className="space-y-1">
                    <motion.div
                      ...
                      whileTap={{ scale: 0.98 }}
                    >
                      <Icon className={`h-5 w-5 ${isParentActive ? 'text-white' : 'text-gray-500'}`} />
                      <span className="font-medium">{item.name}</span>
                    </motion.div>

                    <div className={`pr-6 pl-4 ${isParentActive ? 'block' : 'hidden'}`}>
                      {item.children.map((child) => {
                        ...
                        return (
                          <motion.button
                            ...
                            className={`w-full flex items-center gap-3 px-4 py-2 rounded-lg text-right transition-all duration-150 ${
                              isActive
                            ...
                            whileTap={{ scale: 0.98 }}
                          >
                            <ChildIcon className={`h-4 w-4 ${isActive ? 'text-green-700' : 'text-gray-400'}`} />
