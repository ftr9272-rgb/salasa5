...
// keep motion import for animated parts used in JSX
...
const SupplierDashboard = () => {
  ...
  // --- Merchants management (Manage Merchants) ---
  ...
  const [partnerType, setPartnerType] = useState('all'); // 'all', 'merchant', 'shipping'

  useEffect(() => {
    // persist locally as fallback
    try {
      localStorage.setItem('supplier_merchants_v1', JSON.stringify(merchants));
    } catch {
      // ignore
    }
  }, [merchants]);

  // Load merchants from backend on mount (non-blocking)
  useEffect(() => {
    let mounted = true;
    fetch('/api/supplier/merchants')
      ...
      .then((data) => {
        if (!mounted) return;
        if (data && data.merchants) setMerchants(data.merchants);
      })
      .catch(() => {
        // keep local fallback with mock data that includes type field
        const mockMerchants = [
          {
            id: 1,
  ...
  const [newMerchant, setNewMerchant] = useState({ name: '', contact: '', phone: '', type: 'merchant' });

  const openAddMerchant = () => {
    setNewMerchant({ name: '', contact: '', phone: '', type: 'merchant' });
    setShowAddMerchant(true);
  };

  const closeAddMerchant = () => setShowAddMerchant(false);

  const handleAddMerchantSubmit = (e) => {
    e.preventDefault();
    if (!newMerchant.name || !newMerchant.name.trim()) {
      alert('الرجاء إدخال اسم التاجر');
      return;
    }
  ...
  fetch('/api/supplier/merchants', {
      ...
      body: JSON.stringify({
        store_name: newMerchant.name,
        email: newMerchant.contact,
      ...
      .then(async (res) => {
        if (!res.ok) {
          ...
          throw new Error(err.error || 'خطأ عند إنشاء التاجر');
        }
        return res.json();
      })
      .then((data) => {
      ...
      .catch(() => {
        ...
        const id = Date.now();
        setMerchants((prev) => [{ id, name: newMerchant.name, contact: newMerchant.contact, phone: newMerchant.phone }, ...prev]);
        setShowAddMerchant(false);
      });
  };
  ...
  const handleDeleteMerchant = (id) => {
    // simple confirmation
    if (!window.confirm('هل تريد حذف هذا التاجر؟')) return;
  ...
  return (
    <div className="p-8 bg-gray-50 min-h-screen">
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-8 mb-8">
        ...
        >
          <div className="flex items-center justify-between mb-6">
            ...
            <button className="text-green-600 font-medium hover:text-green-700 flex items-center transition-colors">
            ...
            {myProducts.map((product, index) => (
              ...
              >
                ...
                <div className="p-6">
                  ...
                  <button className="w-full bg-green-500 text-white py-2 rounded-full font-semibold hover:bg-green-600 transition-colors text-sm">
                    تعديل المنتج
                  </button>
                </div>
              </motion.div>
      ...
      >
        <div className="flex items-center justify-between mb-4">
          <div className="flex items-center space-x-4">
            <h3 className="text-xl font-bold text-gray-800">إدارة الشركاء</h3>
            <div className="flex items-center space-x-2 bg-white border border-gray-200 rounded-lg shadow-sm p-1">
              <button 
                ...
                className={`px-4 py-2 rounded-md text-sm font-medium transition-colors ${partnerType === 'all' ? 'bg-blue-500 text-white shadow-md' : 'text-gray-700 hover:bg-gray-100'}`}
              >
                الكل
              </button>
              <button 
                onClick={() => setPartnerType('merchant')} 
                className={`px-4 py-2 rounded-md text-sm font-medium flex items-center transition-colors ${partnerType === 'merchant' ? 'bg-green-500 text-white shadow-md' : 'text-gray-700 hover:bg-gray-100'}`}
              >
                <Users className="w-4 h-4 mr-1" />
                التجار
              </button>
              <button 
                onClick={() => setPartnerType('shipping')} 
                className={`px-4 py-2 rounded-md text-sm font-medium flex items-center transition-colors ${partnerType === 'shipping' ? 'bg-blue-500 text-white shadow-md' : 'text-gray-700 hover:bg-gray-100'}`}
              >
          ...
          <div className="flex items-center space-x-2">
            {partnerType !== 'shipping' && (
              <button onClick={openAddMerchant} className="bg-green-600 text-white px-4 py-2 rounded-full hover:bg-green-700 transition-colors flex items-center">
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
          {(() => {
            const filteredMerchants = merchants.filter(m => partnerType === 'all' || m.type === partnerType);
            return filteredMerchants.length === 0 ? (
              <div className="text-gray-500">لا توجد شركاء مسجلة حتى الآن.</div>
            ) : (
              filteredMerchants.map((m) => (
                <div key={m.id || m.user_id || m.store_name} className="bg-gray-50 p-4 rounded-xl border border-gray-100 flex items-start justify-between">
                  <div>
                    <div className="flex items-center mb-1">
                      <h4 className="font-medium text-gray-800">{m.store_name || m.name}</h4>
                      <span className={`ml-2 text-xs px-2 py-1 rounded-full ${m.type === 'merchant' ? 'bg-green-100 text-green-800' : 'bg-blue-100 text-blue-800'}`}>
                        {m.type === 'merchant' ? 'تاجر' : 'شحن'}
                      </span>
                    </div>
                    <p className="text-sm text-gray-500">{m.email || m.contact || '-'}</p>
                    <p className="text-sm text-gray-500">{m.phone || '-'}</p>
                    {m.businessType && <p className="text-sm text-gray-500 mt-1">النوع: {m.businessType}</p>}
                  </div>
                  <div className="flex flex-col items-end">
                    <button onClick={() => handleDeleteMerchant(m.id || m.user_id)} className="text-red-600 bg-red-50 px-3 py-1 rounded-md hover:bg-red-100">
                      حذف
                    </button>
                  </div>
                </div>
    ...
    {showAddMerchant && (
      <div className="fixed inset-0 z-50 flex items-center justify-center bg-black bg-opacity-40">
        <div className="bg-white rounded-2xl w-full max-w-md p-6 shadow-xl">
          <h4 className="text-lg font-bold mb-4">إضافة تاجر جديد</h4>
          <form onSubmit={handleAddMerchantSubmit} className="space-y-4">
            <div>
              <label className="text-sm text-gray-700">اسم التاجر</label>
              <input autoFocus value={newMerchant.name} onChange={(e) => setNewMerchant((s) => ({ ...s, name: e.target.value }))} className="w-full mt-2 px-3 py-2 border rounded-lg" />
            </div>
            <div>
              <label className="text-sm text-gray-700">البريد الإلكتروني</label>
              <input value={newMerchant.contact} onChange={(e) => setNewMerchant((s) => ({ ...s, contact: e.target.value }))} className="w-full mt-2 px-3 py-2 border rounded-lg" />
            </div>
            <div>
              <label className="text-sm text-gray-700">الهاتف</label>
              <input value={newMerchant.phone} onChange={(e) => setNewMerchant((s) => ({ ...s, phone: e.target.value }))} className="w-full mt-2 px-3 py-2 border rounded-lg" />
            </div>
            <div className="flex items-center justify-end gap-3">
              <button type="button" onClick={closeAddMerchant} className="px-4 py-2 rounded-lg bg-gray-100">إلغاء</button>
