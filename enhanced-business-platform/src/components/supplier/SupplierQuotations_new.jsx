...
const SupplierQuotations = () => {
  ...
  const [newOffer, setNewOffer] = useState({ merchantName: '', items: '', price: '', deadline: '', notes: '' });

  const openAddOffer = () => {
    setNewOffer({ merchantName: '', items: '', price: '', deadline: '', notes: '' });
    setShowAddOffer(true);
  };

  const closeAddOffer = () => setShowAddOffer(false);

  const handleAddOfferSubmit = async (e) => {
    e.preventDefault();
    if (!newOffer.merchantName || !newOffer.merchantName.trim()) {
      alert('الرجاء إدخال اسم التاجر');
      return;
    }
    const payload = {
      merchant_name: newOffer.merchantName,
      items: newOffer.items.split(',').map(s => s.trim()).filter(Boolean),
      total_amount: newOffer.price ? parseFloat(newOffer.price) : null,
      valid_until: newOffer.deadline,
  ...
  const res = await fetch('/api/supplier/quotations', {
      ...
      const created = data && (data.quotation || data.quotations || data);
      const id = (created && created.id) || Date.now();
      const newQ = {
        id,
        quotationNumber: created && (created.quotation_number || created.quotationNumber) || `QUO-${id}`,
        merchant: { name: payload.merchant_name },
        requestedItems: payload.items.map((it, i) => ({ name: it, quantity: 1 })),
        status: 'quoted',
        requestDate: new Date().toISOString().split('T')[0],
        deadline: payload.valid_until || '',
        notes: payload.notes || '',
        quotedPrice: payload.total_amount
      };
      ...
      const newQ = {
        ...
        merchant: { name: payload.merchant_name },
        requestedItems: payload.items.map((it) => ({ name: it, quantity: 1 })),
        status: 'quoted',
        requestDate: new Date().toISOString().split('T')[0],
        deadline: payload.valid_until || '',
        notes: payload.notes || '',
      ...
      >
        <Card>
          <CardHeader className="flex items-center justify-between">
            ...
            <div>
              <Button onClick={openAddOffer} className="bg-green-600 text-white hover:bg-green-700">
      ...
      {showAddOffer && (
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black bg-opacity-40">
          <div className="bg-white rounded-2xl w-full max-w-md p-6 shadow-xl">
            <h4 className="text-lg font-bold mb-4">إضافة عرض سعر</h4>
            <form onSubmit={handleAddOfferSubmit} className="space-y-4">
              <div>
                <label className="text-sm text-gray-700">اسم التاجر</label>
                <input autoFocus value={newOffer.merchantName} onChange={(e) => setNewOffer(s => ({ ...s, merchantName: e.target.value }))} className="w-full mt-2 px-3 py-2 border rounded-lg" />
              </div>
              <div>
                <label className="text-sm text-gray-700">المنتجات (افصل بعلامة ,)</label>
                <input value={newOffer.items} onChange={(e) => setNewOffer(s => ({ ...s, items: e.target.value }))} className="w-full mt-2 px-3 py-2 border rounded-lg" />
              </div>
              <div className="grid grid-cols-2 gap-3">
                <div>
                  <label className="text-sm text-gray-700">السعر الإجمالي</label>
                  <input value={newOffer.price} onChange={(e) => setNewOffer(s => ({ ...s, price: e.target.value }))} className="w-full mt-2 px-3 py-2 border rounded-lg" />
                </div>
                <div>
                  <label className="text-sm text-gray-700">صلاحية العرض (تاريخ)</label>
                  <input type="date" value={newOffer.deadline} onChange={(e) => setNewOffer(s => ({ ...s, deadline: e.target.value }))} className="w-full mt-2 px-3 py-2 border rounded-lg" />
                </div>
              </div>
              <div>
                <label className="text-sm text-gray-700">ملاحظات</label>
                <textarea value={newOffer.notes} onChange={(e) => setNewOffer(s => ({ ...s, notes: e.target.value }))} className="w-full mt-2 px-3 py-2 border rounded-lg" />
              </div>
              <div className="flex items-center justify-end gap-3">
                <button type="button" onClick={closeAddOffer} className="px-4 py-2 rounded-lg bg-gray-100">إلغاء</button>
