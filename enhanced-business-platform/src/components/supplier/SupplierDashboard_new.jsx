...
const SupplierDashboard = () => {
  ...
  const [merchants, setMerchants] = useState(() => {
    try {
      const raw = localStorage.getItem('supplier_merchants_v1');
  ...
  const [newMerchant, setNewMerchant] = useState({ name: '', contact: '', phone: '' });

  const openAddMerchant = () => {
    setNewMerchant({ name: '', contact: '', phone: '' });
  ...
  const handleDeleteMerchant = (id) => {
    ...
    setMerchants((prev) => prev.filter((m) => m.id !== id && m.user_id !== id));
  };
  ...
  return (
    ...
    {showAddMerchant && (
      ...
      >
        <div className="flex items-center justify-between mb-4">
          <h3 className="text-xl font-bold text-gray-800">إدارة التجار</h3>
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
          {merchants.length === 0 ? (
            <div className="text-gray-500">لا توجد تجار مسجلة حتى الآن.</div>
          ) : (
            merchants.map((m) => (
