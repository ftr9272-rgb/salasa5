...
// module-level mock data for shipping companies
const MOCK_SHIPPING_QUOTATIONS = [
  {
    id: 1,
...
const MerchantShippingCompanies = () => {
  ...
  // mock data is declared at module-level (MOCK_SHIPPING_QUOTATIONS)
  // mock data is declared at module-level (MOCK_SHIPPING_QUOTATIONS)

  const [reloadCounter, _setReloadCounter] = useState(0);
  useEffect(() => {
    const fetchQuotations = async () => {
      setLoading(true);
      try {
        await new Promise(resolve => setTimeout(resolve, 800));
        setQuotations(MOCK_SHIPPING_QUOTATIONS);
      } catch (error) {
    ...
    return (
      <div className="p-6 bg-gray-50 min-h-screen flex items-center justify-center">
        <div className="text-center max-w-md p-6 bg-white rounded-lg shadow-md border border-red-200">
          ...
          <button
            onClick={() => _setReloadCounter(prev => prev + 1)}
            className="px-4 py-2 bg-blue-500 text-white rounded-lg hover:bg-blue-600 transition-colors"
          >
            إعادة المحاولة
          </button>
  ...
  <h1 className="text-3xl font-bold text-gray-800 dark:text-white">شركات الشحن</h1>
...
export default MerchantShippingCompanies;
